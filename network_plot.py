...
from bokeh.models import TapTool, OpenURL, HoverTool, CustomJS
...
canis_data["Parent entity (English) Children #"] = (
    canis_data.groupby("Parent entity (English)")["Name (English)"]
    .transform("count")
    .astype(float)
)
...
edges_lvl_1 = [
    tuple(row)
    for row in canis_data[
        ["Parent entity (English)", "Entity owner (English)"]
    ].itertuples(index=False, name=None)
]

edges_lvl_2 = [
    tuple(row)
    for row in canis_data[["Entity owner (English)", "Name (English)"]].itertuples(
        index=False, name=None
    )
]

G.add_edges_from(edges_lvl_1)
G.add_edges_from(edges_lvl_2)
...
for node in G.nodes:
    ...
    if node in canis_data["Name (English)"].unique():
        G.nodes[node]["url"] = canis_data[canis_data["Name (English)"] == node][
            "X (Twitter) URL"
        ].iloc[0]
        ...
        G.nodes[node]["size"] = (
            np.sqrt(
                (
                    canis_data[canis_data["Parent entity (English)"] == node][
...
# Render the plot
labelled_graph = (hv_graph * labels).opts(
    opts.Labels(text_font_size="10pt", visible=False)
)
...
label_callback = CustomJS(
    args=dict(labels=plot.renderers[-1]),
    code=f"""
